...
class RallyValidate {
  ...
  async handlePullRequest (context) {
    // Initialize Rally Work Items
    const rallyWorkItems = {}
    ...
    try {
      ...
      rallyWorkItems.commits = await this.checkCommitMessages(context, config, rallyClient)
      // Get the PR title for validation
      rallyWorkItems.titleList = await this.checkPRTitle(context, config, rallyClient)
      // Get the PR body for validation
      rallyWorkItems.bodyList = await this.checkPRBody(context, config, rallyClient)
      // Get the PR labels for validation
      rallyWorkItems.labelList = await this.checkPRLabels(context, config, rallyClient)
    } catch (e) {
      await this.setStatusFail(context, 'Error occurred while validating Rally Work Items: ' + e)
      return
    }

    // Set the status message
    const { statusMessage, isSuccess } = await this.processIssues(config, rallyWorkItems)
    // Set the check status based on Rally issue validation
    if (isSuccess) {
  ...
  createStatusMessage ({ checkPRLabels, checkPRBody, checkPRTitle, checkCommitMessages, statuses, bodyMessage, titleMessage, labelMessage, commitsMessage }) {
    return outdent`This project requires a valid Rally work item to be present in the following portions of this pull before merge will be allowed:
    ...
    _Valid issue states for this project_:
  ...
  // Process the issues
  async processIssues (config, rallyWorkItems) {
    const statuses = config.issueStatuses.map(status => `- [x] \`${status}\``)
    ...
    if (config.checkPRBody) {
      bodyResult = await this.formatBodyMessage(config, rallyWorkItems.bodyList)
    }
    if (config.checkPRTitle) {
      titleResult = await this.formatTitleMessage(config, rallyWorkItems.titleList)
    }
    if (config.checkPRLabels) {
      labelResult = await this.formatLabelMessage(config, rallyWorkItems.labelList)
    }
    if (config.checkCommitMessages) {
      commitsResult = await this.formatCommitsMessage(config, rallyWorkItems.commits.commitsWithIssue, rallyWorkItems.commits.commitsWithoutIssue)
    }
  ...
  async formatLabelMessage (config, labelsList) {
    ...
    if (labelsList && labelsList.length > 0) {
      ...
      message += '| Issue | Rally Status | Validation |\n'
      message += '| --- | --- | --- |\n'
      labelsList.forEach(issue => {
        const status = config.issueStatuses.includes(issue.status)
        ...
        message += `| ${issue.key} | \`${issue.status}\` | ${statusIcon} \`${validState}\` |\n`
      })
    } else {
      message += '\n:heavy_exclamation_mark: No valid issues were found in the pull request labels'
      isSuccess = false
    }
  ...
  async formatBodyMessage (config, bodyList) {
    ...
    if (bodyList && bodyList.length > 0) {
      message += 'The following Rally work items have been found in the body of this pull request, with validation status below\n\n'
      message += '| Work Item | Rally Status | Validation |\n'
      message += '| --- | --- | --- |\n'
      bodyList.forEach(issue => {
        const status = config.issueStatuses.includes(issue.status)
        const validState = status ? 'passed' : 'failed'
        const statusIcon = status ? ':heavy_check_mark:' : ':heavy_exclamation_mark:'
        if (!status) { isSuccess = false }
      ...
      message += '\n:heavy_exclamation_mark: No valid issues were found in the pull request body'
      isSuccess = false
    }
    return { message, isSuccess }
  }
  ...
  */
  async formatTitleMessage (config, titleList) {
    ...
    if (titleList && titleList.length > 0) {
      message += 'The following Rally issues have been found in the title of this pull request, with validation status below\n\n'
      message += '| Issue | Rally Status | Validation |\n'
      message += '| --- | --- | --- |\n'
      titleList.forEach(issue => {
        const status = config.issueStatuses.includes(issue.status)
        const validState = status ? 'passed' : 'failed'
        const statusIcon = status ? ':heavy_check_mark:' : ':heavy_exclamation_mark:'
        if (!status) { isSuccess = false }
  ...
  * @param config
  * @param commitsWithIssue
  * @param commitsWithoutIssue
  */
  async formatCommitsMessage (config, commitsWithIssue = [], commitsWithoutIssue = []) {
    // Default isSuccess to true, then set to fail if matches fail
    let isSuccess = true

    // Set the content headers
    let message = '\n### Commit validation\n'
    message += '| Issue | Commit SHA | Rally Status | Validation |\n'
    message += '| --- | --- | --- | --- |\n'
    // Process commits with Rally issues in them
    commitsWithIssue.forEach(issue => {
      const status = config.issueStatuses.includes(issue.rally.status)
      const validState = status ? 'passed' : 'failed'
      const statusIcon = status ? ':heavy_check_mark:' : ':heavy_exclamation_mark:'
      if (!status) { isSuccess = false }
      // Append the status to the PR comment
      message += `| ${issue.rally.key} | [${issue.sha_short}](${issue.commit_url}) | \`${issue.rally.status}\` | ${statusIcon} \`${validState}\` |\n`
    })
    // Process commits without Rally issues
    if (commitsWithoutIssue && commitsWithoutIssue.length > 0) {
      let commitsColumn = ''
      commitsWithoutIssue.forEach(commit => {
  ...
  async checkCommitMessages (context, config, rallyClient) {
    const commitsWithIssue = []
    const commitsWithoutIssue = []
    ...
    if (config.checkCommitMessages) {
      ...
      // Check that every commit message has a Rally issue in it
      for (const data of compare.data.commits) {
        const rallyWorkItem = this.findWorkItem(data.commit.message, config.rallyObjects)
        const commitStatus = {
          sha: data.sha,
          message: data.commit.message,
          sha_short: data.sha.substr(0, 6),
          commit_url: `${commitsUrl}/commits/${data.sha}`,
          rally: await this.validateWorkItem(rallyClient, rallyWorkItem[0], 'commitMessages', config)
        }
        // Group commits with keys and those without
        if (commitStatus.rally) {
          commitsWithIssue.push(commitStatus)
        } else {
          commitsWithoutIssue.push(commitStatus)
        }
      }
      return { commitsWithIssue, commitsWithoutIssue }
    }
  }

  async checkPRTitle (context, config, rallyClient) {
    if (config.checkPRTitle) {
      // Look for issue keys in the PR title
      const issueKeys = this.findWorkItem(context.payload.pull_request.title, config.rallyObjects)
      if (issueKeys) { // validate each key we find
        return Promise.all(issueKeys.map(issueKey => {
          return this.validateWorkItem(rallyClient, issueKey, 'prTitle', config)
        }))
      } else {
        this.robot.log.debug('No issue found in PR title')
      }
    }
  }
  ...
  async checkPRBody (context, config, rallyClient) {
    if (config.checkPRBody) {
      // Look for issue keys in the PR body
      const issueKeys = this.findWorkItem(context.payload.pull_request.body, config.rallyObjects)
      if (issueKeys) { // validate each key we find
        return Promise.all(issueKeys.map(issueKey => {
          return this.validateWorkItem(rallyClient, issueKey, 'prBody', config)
        }))
      } else {
        this.robot.log.debug('No issue found in PR body')
      }
    }
  }
  ...
  async checkPRLabels (context, config, rallyClient) {
    if (config.checkPRLabels) {
      // Create an empty array for storing all valid issue ID's
      const issueKeys = []
      // Check each label to see if it contains an issue ID
      for (const label of context.payload.pull_request.labels) {
        const issueKey = this.findWorkItem(label.name, config.rallyObjects)
        if (issueKey) {
          issueKeys.push(...issueKey)
        }
      }
      // Determine if we found any issue ID's
      if (issueKeys && issueKeys.length > 0) { // Validate any found issues
        return Promise.all(issueKeys.map(issueKey => {
          return this.validateWorkItem(rallyClient, issueKey, 'prLabel', config)
        }))
      } else {
        this.robot.log.debug('No issue found in PR labels')
      }
    }
  }
  ...
  async setStatusPending (context) {
    return context.github.checks.create(context.repo({
      ...
      output: {
        ...
        summary: 'We\'re currently validating the status of any Rally work items associated with this pull request. Please stand by.'
      }
    }))
  }
  ...
  async setOverridePass (context) {
    ...
    return context.github.checks.create(context.repo({
      ...
      output: {
        title: 'Rally issue validation manually overridden by @' + context.payload.sender.login,
        summary: 'Commit sign-off was manually approved by @' + context.payload.sender.login
      }
    }))
  }

  async setStatusPass (context, statusMessage = 'All Rally issues have been validated!') {
    const timeStart = new Date()
    return context.github.checks.create(context.repo({
      name: 'rally/issue-validator',
      head_branch: context.payload.pull_request.head.ref,
      ...
      completed_at: new Date(),
      output: {
        title: 'Rally issues have been validated',
        summary: statusMessage
      }
    }))
  }

  async setStatusFail (context, statusMessage = 'Please provide a valid Rally issue') {
    const timeStart = new Date()
    return context.github.checks.create(context.repo({
      name: 'rally/issue-validator',
      head_branch: context.payload.pull_request.head.ref,
      ...
      completed_at: new Date(),
      output: {
        title: 'Rally issue validation failed',
        summary: statusMessage
      }
    }))
  }

  async validateWorkItem (rallyClient, key, property, config) {
    try {
      if (key !== null) {
        const githubWorkItem = this.parseWorkItem(key)
        const workItems = await rallyClient.query({
          type: githubWorkItem.rallyType,
          start: 1,
          pageSize: 2,
          limit: 20,
          order: 'Rank',
          ...
          query: queryUtils.where('FormattedID', '=', githubWorkItem.number),
          requestOptions: {}
        })
        let status
        let validState
        if (workItems.TotalResultCount === 0) {
          ...
          validState = config.issueStatuses.includes(status) ? 'passed' : 'failed'
        }
      ...
      try {
        ...
        if (eJson.body && eJson.body.errorMessages.length > 0) {
          const issueStatus = { key: key, property: property, status: eJson.body.errorMessages[0] }
          return issueStatus
        }
      } catch (jsonError) {
  ...
  createNoConfigMessage () {
    ...
    # Check PR Labels for Rally issue
    checkPRLabels: true

    # Check PR Body for Rally issue
    checkPRBody: true

    # Check PR Title for Rally issue
    checkPRTitle: true

    # Check all commit messages for a Rally issue
  ...
  findWorkItem (text, workItemTypes) {
    ...
    prefixes.forEach(prefix => {
      ...
      const issueMatches = text.match(regexp)
      if (issueMatches) {
        issues = issues.concat(issueMatches)
      }
    })
    return issues
  }
  ...
  async rerunCheck (context) {
    ...
    if (context.name === 'check_suite') {
      ...
      if (checkName !== 'rally/issue-validator') {
        return
      }
    }
  ...
  parseWorkItem (workItemName) {
    ...
    workItem.prefix = workItemName.match(/([A-Z]{1,2})/i)[0]
    workItem.number = workItemName.match(/[1-9].*/)[0]
    workItem.rallyType = typeMapping[workItem.prefix]

    return workItem
  }
}

// getState(workItem, config) {
//   const validStates = config.
// }
