...
class RallyValidate {
  ...
  async handlePullRequest (context) {
    ...
    const noFile = true
    let orgConfig
    const orgConfigRepoName = process.env.ORG_CONFIG_REPO_NAME ? process.env.ORG_CONFIG_REPO_NAME : 'rally-validator-config'

    if (orgConfigRepoName !== undefined) {
      const orgConfigResponse = await context.github.repos.getContents({
        owner: context.payload.pull_request.base.repo.owner.login, // GitHub user who created Pull Request
        repo: orgConfigRepoName, // GitHub repo name
        path: '.github/' + configFile, // Path to the config file
        ref: 'master' // Branch of code
      })
        .catch(() => ({
          // Failed to find or open the config file
          noFile
        }))
    ...
    // Get our config file
    const config = await context.config(configFile, orgConfig)
