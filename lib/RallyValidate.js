...
class RallyValidate {
  ...
  async initializeRallyClient (config) {
    ...
    const rallyClient = rally({
      user: process.env.RALLY_USERNAME, // Required if no api key, defaults to process.env.RALLY_USERNAME
      pass: process.env.RALLY_PASSWORD, // Required if no api key, defaults to process.env.RALLY_PASSWORD
    ...
    try {
      if (key !== null && key !== undefined) {
        ...
        if (workItems.TotalResultCount === 0) {
          ...
          projectName = workItems.Results[0].Project._refObjectName
