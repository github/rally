...
function getVarConfig() {
  const contents = fs.readFileSync(path.join(__dirname, '..', 'variables.yml'))
  const variables = yaml.safeLoad(contents)
...
function serialize(env) {
  ...
  return Object.keys(env).map(key => `${key}=${env[key].replace(/\r?\n/g, '\\n')}`).join(os.EOL)
}
